...
for s, state in enumerate(states):
    ...
    domain.render(state, ax, x0=s*9, y0=0, txt=False)
    if s + 1 < len(states):
...
for a,action in [(-5, (1,1,1)), (5, (2,1,1))]:
    domain.render(domain.perform(action, states[s]), ax, x0=(s+1)*9 - 3, y0=a, txt=False)
...
for n,s in enumerate([0, 3]):
    ...
    domain.render(states[s]*(1-w), ax, x0=s*9, y0=-10, txt=False)
